...
# =============================================================================
# Free LLM setup — Aggressive Environment Injection for CrewAI
# =============================================================================

#def _get_gemini_llm(model: str = "gemini-2.0-flash"):
#def _get_gemini_llm(model: str = "gemini-1.5-flash"):
def _get_gemini_llm(model: str = "gemini-1.5-pro"):
    """
    Final, bulletproof Gemini connection for CrewAI.
    Forces model strings and satisfies the OpenAI-only validator.
    """
    ...
    if not api_key:
        print("  [LLM] GOOGLE_API_KEY not found in .env file.")
        print("  [LLM] Falling back to offline rule-based report generation.\n")
        return None

    try:
        # 1. FORCE THE ENVIRONMENT - This is critical for LiteLLM (CrewAI's engine)
        os.environ["GEMINI_API_KEY"] = api_key
        os.environ["GOOGLE_API_KEY"] = api_key
        
        # 2. TRICK THE VALIDATOR - CrewAI often refuses to start without this string
        os.environ["OPENAI_API_KEY"] = "sk-fake-key-to-bypass-internal-validator"
        
        # 3. USE THE NATIVE LLM CLASS - This is the most stable method in CrewAI v0.100+
        from crewai import LLM
        
        # We use 'gemini/gemini-1.5-pro' because it is the most stable endpoint for v1beta
        llm = LLM(
        ...
        print(f"  [LLM] ADFI Multi-Agent Protocol initiated via Gemini 1.5 Pro.\n")
        return llm
...
def run_layer3_intelligence(
    ...
    print("=" * 65)
    print("  LAYER 3 — INTELLIGENCE & ACTION LAYER (XAI + CrewAI + Gemini)")
    print("=" * 65 + "\n")
    ...
    if llm is not None:
        try:
            ...
            if crew: full_report = str(crew.kickoff())
        except Exception as e:
