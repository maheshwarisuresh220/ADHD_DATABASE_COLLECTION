...
POSE_MODEL = os.path.join(APP_DIR, "pose_landmarker_lite.task")
FACE_MODEL = os.path.join(APP_DIR, "face_landmarker.task")
...
@st.cache_resource
def download_models():
    """Download MediaPipe task models once into the app directory."""
    for path, url in [(POSE_MODEL, POSE_URL), (FACE_MODEL, FACE_URL)]:
        ...
        try:
            ...
            if not os.path.exists(tmp_path) or os.path.getsize(tmp_path) < 1024:
                raise RuntimeError(f"Downloaded model is missing or too small: {os.path.basename(path)}")
            ...
            try:
                ...
                pass
            raise RuntimeError(f"Could not download MediaPipe model {os.path.basename(path)}: {exc}") from exc
...
def protocol_hash(trials):
    payload = json.dumps(trials, sort_keys=True, separators=(",", ":"), ensure_ascii=True).encode("utf-8")
    ...
    def __init__(self):
        ...
        try:
            pose_options = vision.PoseLandmarkerOptions(
                ...
                min_pose_detection_confidence=0.40,
            ...
            raise RuntimeError(
                "MediaPipe native runtime could not initialize. "
                f"Python={os.sys.version.split()[0]}, MediaPipe={getattr(mp, '__version__', 'unknown')}. "
                "Use the supplied requirements.txt with Python 3.13 and redeploy the Streamlit app. "
        ...
        try:
            ...
            try:
                ...
                if pose_result and pose_result.pose_landmarks:
                    ...
                    v, a, mv = self._process_kinematics(p, monotonic_now)
                ...
                if face_result and face_result.face_landmarks:
                    valid_face = True
                    blink, gaze_dev, prolonged_occlusion = self._process_face(face_result.face_landmarks[0], monotonic_now)
...
def validate_cpt_dataset(trials):
    ...
    for t in trials:
        required = {"phase", "target", "scored", "stimulus", "interval", "stimulus_onset_epoch"}
        missing = [k for k in required if k not in t]
        if missing:
            return False, f"Trial {t.get('trial_id')} is missing required fields: {', '.join(missing)}."
...
def save_to_local_pc(participant, session_id, summary, trials, intervals, raw_frames):
    ...
    try:
        ...
        # --------------------------------------------------------
        complete_dataset = {
            "schema_version": "ADFI-Research-v6",
...
def build_complete_download_package(participant, session_id, summary, trials, intervals, raw_frames):
    ...
    complete_dataset = {
        ...
        "summary": summary_safe,
        "integrity": firestore_safe(st.session_state.get("integrity_events", {})),
        "protocol": {
            "total_trials": TOTAL_TARGET_TRIALS,
            "training_trials": 40,
            "scored_trials": SCORED_TRIALS,
...
(function(){
  ...
  let enabled=false, hiddenCount=0, hiddenStarted=null, hiddenTotalMs=0, blurCount=0, focusCount=0, protocolHash="";
  ...
  function finish(){
    ...
    if(document.visibilityState === "hidden" && hiddenStarted!==null){hiddenTotalMs += performance.now()-hiddenStarted; hiddenStarted=null;}
    value({status:"completed",total_trials:results.length,results:results,integrity:{hidden_count:hiddenCount,hidden_duration_ms:Math.round(hiddenTotalMs),blur_count:blurCount,focus_count:focusCount,protocol_hash:protocolHash}});
  }
  function start(){
    if(running||finished||!enabled)return;
    if(!Array.isArray(trials)||trials.length!==480){msg.textContent="Protocol error: expected 480 trials.";return}
    if(!trials.every((t,i)=>Number(t.trial_id)===i+1)){msg.textContent="Protocol integrity error: invalid trial sequence.";return}
  ...
  function render(e){
    ...
    protocolHash=String(a.protocol_hash||"");
    if(!enabled && !running && !finished){msg.textContent="Camera check required before the assessment can start.";}
    height();if(enabled&&!running&&!finished)start();
  }
  ...
  document.addEventListener("visibilitychange",function(){
    if(document.visibilityState === "hidden"){
      hiddenCount++; hiddenStarted=performance.now();
    }else if(hiddenStarted!==null){
      hiddenTotalMs += performance.now()-hiddenStarted; hiddenStarted=null;
    }
  });
  window.addEventListener("blur",function(){blurCount++;});
  window.addEventListener("focus",function(){focusCount++;});
    ...
    if not st.session_state.started:
        ...
        with right:
            ...
            try:
                ...
                ctx = None
                st.error("Camera processing could not start on the research server.")
                st.code(
                    f"{type(exc).__name__}: {exc}\n\n"
                    "This deployment must use Python 3.13 and the pinned MediaPipe build. "
                    "Redeploy after adding the supplied requirements.txt and selecting Python 3.13.",
                    language="text"
                )
                st.stop()
        ...
        with left:
            ...
            result = adfi_cpt_component(
                ...
                enabled=bool(st.session_state.camera_ready),
                protocol_hash=protocol_hash(st.session_state.cpt_trials),
                height=470,
            ...
            if isinstance(result, dict):
                ...
                if status == "completed":
                    ...
                    returned_hash = str((result.get("integrity") or {}).get("protocol_hash", ""))
                    expected_hash = protocol_hash(st.session_state.cpt_trials)
                    if returned_hash and returned_hash != expected_hash:
                        valid = False
                        message = "Protocol integrity hash mismatch. Session rejected."

                    if valid:
                        st.session_state.component_result = result
                        st.session_state.integrity_events = result.get("integrity", {}) or {}
    ...
    # ========================================================
        ...
        if not st.session_state.data_processed:
            ...
            try:
                ...
                summary["integrity"] = firestore_safe(st.session_state.get("integrity_events", {}))
